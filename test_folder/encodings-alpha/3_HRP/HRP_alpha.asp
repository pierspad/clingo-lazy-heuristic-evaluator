%% ============================================================
%% HRP per Alpha - PRESO TALE E QUALE dai materiali dei paper.
%%
%% Origine: ALPHA/src/test/resources/DomainHeuristics/House/
%%          house_alpha_2021-02-04.asp
%% (encoding degli autori per "Domain-specific Heuristics in ASP",
%% Comploi-Taupe et al., dove HRP e' la Case Study 1 della Sezione 6.2).
%% NON e' una traduzione fatta qui: e' il file degli autori, copiato senza
%% modifiche - sotto questa intestazione il contenuto e' byte per byte
%% quello originale, verificabile con `diff` contro il file in ALPHA/.
%%
%% DIFFERENZA IMPORTANTE RISPETTO A BSP/PUP: qui le euristiche NON usano
%% aggregati dinamici. Le loro condizioni sono letterali normali e pesi
%% aritmetici (`not fullCabinet(C)`, `maxCabinet(MC), W=MC-C`); l'aggregato
%% c'e' ma sta una regola piu' in la', nella definizione di fullCabinet/
%% fullRoom. E' esattamente la stessa forma dell'euristica riprodotta da
%% HRP_la.lp / HRP_lc.lp di questa suite, che di quel file sono un port
%% riga per riga. Quindi HRP misura ground-and-solve contro lazy grounding
%% a parita' di euristica, SENZA la variabile "aggregati dinamici" che su
%% BSP e PUP e' invece il punto centrale: e' un utile controllo che separa
%% i due effetti, non un doppione di quei due confronti.
%%
%% Di conseguenza -uqh (euristiche valutate come query Prolog) qui non ha
%% niente di dinamico da valutare: v. i due setting alpha_hrp/alpha nel
%% runscript, che coprono entrambe le modalita'.
%%
%% Le istanze house-*.asp della suite scrivono i fatti legacy con nomi
%% piatti (legacyCabinet/1, legacyRoomCabinet/2, ...), questo encoding li
%% legge come termini dentro legacyConfig/1. Il raccordo e' in
%% HRP_alpha_bridge.asp, che va passato insieme a questo file: e' pura
%% rinomina, nessuna regola di modellazione.
%% ============================================================
