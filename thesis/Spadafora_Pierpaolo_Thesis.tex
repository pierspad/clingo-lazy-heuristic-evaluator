...
\begin{lstlisting}[caption={Codifica standard con euristica ground (file \texttt{BSP\_gc.lp})}]
b(X) :- x(X), not c(X).
c(X) :- x(X), not b(X).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{../test_folder/results/graphs/bsp/standard/benchmark_results.png}
