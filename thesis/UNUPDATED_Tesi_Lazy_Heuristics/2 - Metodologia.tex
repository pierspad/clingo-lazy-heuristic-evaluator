...
The flattening rule interacts with the two axes as follows, and this interaction is enforced consistently across the suite. In the ground variants (\mintinline{text}|gc|, \mintinline{text}|ga|), clasp's Domain heuristic is the consumer, so flattening is always required. In the lazy variants the consumer is the propagator, whose ranking behaviour is part of the semantics being modelled: under \mintinline{text}|__semantics(alpha)| the native backend ranks candidates globally by (priority, weight), faithfully reproducing Alpha's levels, so \mintinline{text}|la| encodings may use native priorities directly; under \mintinline{text}|__semantics(clingo)| the native backend deliberately reproduces clingo's local-only reading of priorities, so \mintinline{text}|lc| encodings must flatten their weights exactly like \mintinline{text}|gc|. The Prolog backend, whose selection rule is a global (priority, weight) sort, expresses the same intended order through its priority field. The net effect is that all four variants of a problem implement the same intended decision order, each through the mechanism appropriate to its semantics.
